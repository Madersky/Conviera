...
interface ConferencesModel extends mongoose.Model<ConferencesDoc> {
  build(attrs: conferenceAttrs): ConferencesDoc;
}

interface ConferencesDoc extends mongoose.Document {
  name: string;
  description: string;
  registrationStartDate: Date;
  registrationEndDate: Date;
...
const Conference = mongoose.model<ConferencesDoc, ConferencesModel>(
  "Conference",
  conferenceSchema
);
